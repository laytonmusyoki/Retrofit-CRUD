from flask import Flask,url_for,render_template,request,flash,redirect,session,make_response
import re
...
def book(id):
    if 'user_id' and 'username' not in session:
        ...
        if request.method=='POST':
            ...
            # send_mail(email,username,name,counsellor,time)
            return redirect(url_for('dashboard'))
        user_id=session['user_id']
        cur = connection.cursor()
        cur.execute("SELECT * FROM schedule WHERE id=%s",(id,))
...
def send_mail(email,username,name,counsellor,time):
...
def print_report():
    ...
    html_content = render_template('report.html', data=data, base_url=base_url)
